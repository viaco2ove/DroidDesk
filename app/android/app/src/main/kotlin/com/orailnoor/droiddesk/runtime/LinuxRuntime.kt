...
class LinuxRuntime(private val context: Context) {
    ...
    fun isUbuntuSshInstalled(): Boolean {
        val prootRootSshd = File(prefixDir, "var/lib/proot-distro/containers/ubuntu/rootfs/usr/sbin/sshd")
        val prootRootMarker = File(prefixDir, "var/lib/proot-distro/containers/ubuntu/rootfs/usr/lib/os-release")
        val legacySshd = File(prefixDir, "var/lib/proot-distro/installed-rootfs/ubuntu/usr/sbin/sshd")
        val legacyMarker = File(prefixDir, "var/lib/proot-distro/installed-rootfs/ubuntu/etc/os-release")
        val hasSshd = prootRootSshd.exists() || legacySshd.exists()
        val hasDistro = prootRootMarker.exists() || legacyMarker.exists()
        return hasSshd && hasDistro
    }
     ...
     */
        ...
        // Android SELinux sandbox blocks reading /proc/*/cmdline of other processes.
        // Pragmatic approach: if sshd port is reachable, proot ubuntu session is alive
        // (sshd only runs inside proot ubuntu). Otherwise probe proot-distro directly.
        if (isUbuntuSshdRunning()) return true
        return try {
            val tmpDirPath = "${tmpDir.absolutePath}/proot"
            java.io.File(tmpDirPath).mkdirs()
            val baseArgs = "login ubuntu " +
                    ...
                    "--env PROOT_LOADER=\"${prefixDir.absolutePath}/libexec/proot/loader\" " +
                    "--env PROOT_LOADER_32=\"${prefixDir.absolutePath}/libexec/proot/loader32\" --"
            // ps -ef inside container sees its own PID namespace + child proot children.
            // If user launched Ubuntu Shell, a bash process is alive inside.
            val output = executeCommand("proot-distro " + baseArgs + " sh -c 'ps -ef 2>/dev/null | grep -E [b]ash | grep -v grep | wc -l'")
            val count = output.trim().toIntOrNull() ?: 0
            val found = count > 0
            Log.d(TAG, "isUbuntuProotRunning (ps inside container): found=" + found + " bashCount=" + count)
            found
        } catch (e: Exception) {
     ...
     */
        ...
        // Android SELinux sandbox blocks /proc/net/tcp and other processes' cmdline.
        // Strategy: open a TCP socket to the SSH port. proot bridges host netns so 127.0.0.1 reaches sshd.
        return try {
            val sp = context.getSharedPreferences("ubuntu_console", android.content.Context.MODE_PRIVATE)
            val port = sp.getString("port", "22")?.toIntOrNull() ?: 22
            // Run on worker thread (CallerRuns policy if pool full); cannot network on main thread.
            ...
            Log.d(TAG, "isUbuntuSshdRunning (TCP probe): found=$found port=$port err=${errorHolder[0]}")
            found
        } catch (e: Exception) {
            Log.w(TAG, "isUbuntuSshdRunning failed: ${e.javaClass.simpleName} ${e.message}")
            false
        }
    }
    ...
    fun startUbuntuSshd(): Boolean {
        ...
        return try {
            ...
            sshdProcess = ProcessBuilder("sh", "-c", fullCmd)
                .redirectErrorStream(true)
                .apply {
                    // 必须传 PATH 和 PROOT 环境变量，否则 sh 找不到 proot-distro 命令
                    environment().apply {
                        ...
                        // proot-distro 用 TERMUX_APP__PACKAGE_NAME 决定锁文件/共享目录路径
                ...
                .start()
            // 启动 consumer 线程读取 sshd 输出，避免 stdout 缓冲区满导致阻塞
            Thread {
                try {
                    sshdProcess!!.inputStream.bufferedReader().forEachLine { line ->
                        Log.i(TAG, "sshd: $line")
                    }
                } catch (e: Exception) {
                    Log.w(TAG, "sshd stdout reader: ${e.message}")
                }
            }.start()
            true
        } catch (e: Exception) {
